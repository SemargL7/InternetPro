...
<main class="mt-1">
    <div class="container-fluid w-75 p-3">
        <div class="d-flex flex-row-reverse">
            <form action="/home/userTariffsList" method="get">
        ...
        <table class="table table-light">
            ...
            <tr>
                ...
                <th id="thPaidStatus" scope="col"><fmt:message key="table.tariff.user.paid-status"/></th>
                <th id="thAction" scope="col"><fmt:message key="opportunity"/></th>

            </tr>
            </thead>
            <tbody>
            <c:forEach var="tariff" items="${userTariff}">
                <tr>
                    <td>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${tariff.status == 0}">
                                <a>Paid</a>
                            </c:when>
                            <c:when test="${tariff.status == 1}">
                                <a>Unpaid</a>
                            </c:when>
                            <c:when test="${tariff.status == 2}">
                                <a>Unknown</a>
                            </c:when>
                        </c:choose>
                    </td>
                    <td style="font-size: large;"><a class="btn btn-outline-dark" title="Disconnect" href="/home/disconnectTariff?id=<c:out value='${tariff.id}' />"><fmt:message key="table.tariff.user.action.disconnect"/></a>
                </tr>
            </c:forEach>
            </tbody>
        </table>
        ...
        <nav>
            <ul class="pagination">
                <li class="page-item">
                    <a class="page-link" href="/home/userTariffsList?page=<c:out value='${page>1?page-1:page}' />" aria-label="Previous">
                ...
                <c:forEach begin="1" end="${paginationMax+1}" varStatus="loop">
                    <c:if test="${loop.index == loop.begin || loop.index == loop.end || (loop.index >= page-3 && loop.index <= page+3)}">
                        <li class="page-item">
                            <a class="page-link" href="/home/userTariffsList?page=<c:out value='${loop.index}' />">${loop.index}<br/></a>
                        </li>
                    </c:if>
                </c:forEach>

                <li class="page-item">
                    <a class="page-link" href="/home/userTariffsList?page=<c:out value='${paginationMax+1 >= page+1? page+1:page}' />" aria-label="Next">
