    ...
    <table class="table table-bordered">
        ...
        <c:forEach var="tariff" items="${userTariff}">
            ...
            <tr>
                ...
                <td style="font-size: large;"><a title="Disconnect" href="/home/disconnectTariff?id=<c:out value='${tariff.id}' />">⇍</a>
            </tr>
        </c:forEach>
        <!-- } -->
        </tbody>
    ...
    <ul id="pagination">
        <c:forEach begin="1" end="${paginationMax+1}" varStatus="loop">
            <c:if test="${loop.index == loop.begin || loop.index == loop.end || (loop.index >= page-3 && loop.index <= page+3)}">
                <li>
                    <a href="/home/userTariffsList?page=<c:out value='${loop.index}' />">${loop.index}<br/></a>
                </li>
            </c:if>
